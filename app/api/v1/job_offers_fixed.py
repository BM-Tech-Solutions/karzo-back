...
def transform_job_offer_to_dict(job_offer, db=None):
    try:
        ...
        # Only count applications if db session is provided
        if db is not None:
            result["applications_count"] = db.query(Interview).filter(Interview.job_offer_id == job_offer.id).count()
        else:
            result["applications_count"] = 0
...
def read_company_job_offers(
    ...
    try:
        ...
        result = [transform_job_offer_to_dict(job, db) for job in job_offers]
...
def update_existing_job_offer(
    ...
    try:
        ...
        result = transform_job_offer_to_dict(updated_job_offer, db)
